...
const Corinto = () => {
  ...
  useMeta({
    ...
    descripcion: 'Piezas arqueológicas de la cultura Corinto en el Museo de Historia Natural Unicauca',
  });
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    setFetchError(null);
    const { data: piezas, error } = await getPiezasPorCultura('Corinto');
    if (error) {
        ...
        {data.length === 0 ? (
          ...
          <>
            <div className='flex flex-col items-center w-full pt-16 gap-1'>
              <label htmlFor="selector-pieza-corinto" className="sr-only">
                Seleccionar pieza arqueológica
              </label>
              <select
                id="selector-pieza-corinto"
                onChange={handleSelectChange}
                aria-describedby="nombre-pieza-corinto"
