...
const Popayan = () => {
  ...
  useMeta({
    ...
    descripcion: 'Piezas arqueológicas de la cultura Popayán en el Museo de Historia Natural Unicauca',
  });
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    setFetchError(null);
    const { data: piezas, error } = await getPiezasPorCultura('Popayán');
    if (error) {
        ...
        {data.length === 0 ? (
          ...
          <>
            <div className='flex flex-col items-center w-full pt-16 gap-1'>
              <label htmlFor="selector-pieza-popayan" className="sr-only">
                Seleccionar pieza arqueológica
              </label>
              <select
                id="selector-pieza-popayan"
                onChange={handleSelectChange}
                aria-describedby="nombre-pieza-popayan"
